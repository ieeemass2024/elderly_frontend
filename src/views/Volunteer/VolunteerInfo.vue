<template>
  <div>
    ...
    <el-table
      ...
      style="width: 100%"
      :default-sort="{prop: 'volunteerId', order: 'ascending'}"
    >
      <el-table-column
        prop="volunteerId"
        label="编号"
      ...
      <el-table-column
        ...
        align="center"
      ></el-table-column>
      <el-table-column prop="gender" label="性别" width="70" header-align="center" align="center"></el-table-column>
      <el-table-column prop="phone" label="电话" width="150" header-align="center" align="center"></el-table-column>
      <el-table-column prop="idCard" label="身份证号" width="180" header-align="center" align="center"></el-table-column>
      <el-table-column
        prop="birthday"
        label="出生日期"
        width="100"
...
export default {
  ...
  methods: {
    ...
    getInfoById(id) {
      ...
      this.$http
        .get("/volunteer/" + id, config)
        .then(response => {
          if (response.data.code === 1) {
            that.form = response.data.data;
            // var volunteerId = response.data.data.volunteerId;
            // that.form.imageUrl =
            //   "https://cos-lqyrmk-1312783534.cos.ap-beijing.myqcloud.com/resources/smart_elderly_care/cv_file/profile/volunteer/" +
            //   volunteerId +
            //   ".jpg";
          } else {
            that.form = {};
        ...
        .then(response => {
          if (response.data.code === 1) {
            ...
            localStorage.setItem(
              "volunteerId",
              JSON.stringify(response.data.data.volunteerId)
            );
    ...
    openModifyDialog(index, row) {
      this.currentId = row.volunteerId;
      this.imageUrl = '';
      ...
      this.$http
        ...
        .then(response => {
          if (response.data.code === 1) {
            ...
            localStorage.setItem(
              "volunteerId",
              JSON.stringify(that.form.volunteerId)
            );
            this.findAll(1);
          } else {
            this.$message({
    ...
    deleteInfo(index, row) {
      ...
      this.$confirm("此操作将永久删除该记录, 是否继续?", "提示", {
        ...
        .then(() => {
          // 发送请求
          this.$http
            .delete("/volunteer/" + row.volunteerId, config)
            .then(response => {
              if (response.data.code === 1) {
                this.$message({
                  type: "success",
    ...
    //上传文件的事件
    uploadFile() {
      var volunteerId = this.currentId;
      ...
      axios
        .post("http://localhost:8090/api/v1/info/upload", formData)
        .then(response => {
          // 处理成功响应
          this.$message({
            type: "success",
